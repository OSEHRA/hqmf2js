class @SpecificsRationaleGenerator

  @generate: (measure, population, rationale, finalSpecifics) ->
    debugger
    return null unless rationale?
    specificsRationaleGenerator = new SpecificsRationaleGenerator(rationale, finalSpecifics)
    specificsRationaleGenerator.specificsRationale()

  constructor: (@rationale, @finalSpecifics) ->

  specificsRationale: ->
    updatedRationale = {}
    orCounts = @calculateOrCounts(@rationale)
    for code in Thorax.Models.Measure.allPopulationCodes
      if specifics = @finalSpecifics[code]
  ...
  codedEntriesForDataCriteria: (dataCriteriaKey) ->
    @rationale[dataCriteriaKey]?['results']
  ...
  codedEntriesPassingSpecifics: (dataCriteriaKey, populationCriteriaKey) ->
    ...
    goodElements = (row[index] for row in @finalSpecifics[populationCriteriaKey]) if index? and @finalSpecifics[populationCriteriaKey]?
