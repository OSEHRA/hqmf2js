...
// isEpisodeInStrat is only relevant for an episode of care measure with stratifications. If this is not the case, isEpisodeInStrat should be false.
root.map = function(record, population, denominator, numerator, exclusion, denexcep, msrpopl, msrpoplex, observ, occurrenceId, isContinuousVariable, stratification, variables, numex, isEpisodeInStrat) {
  ...
  if (isContinuousVariable) {
    value = calculateCV(record, population, msrpopl, msrpoplex, observ, occurrenceId, value, stratification, variables, isEpisodeInStrat)
  } else {
    value = calculate(record, population, denominator, numerator, exclusion, denexcep, occurrenceId, value, stratification, variables, numex, isEpisodeInStrat)
  }
...
root.calculate = function(record, population, denominator, numerator, exclusion, denexcep, occurrenceId, value, stratification, variables, numex, isEpisodeInStrat) {
  ...
  if (ipp) {
    ...
    if (hqmf.SpecificsManager.validate(ipp)) {
      ...
      if (validStrat && value.STRAT == 0 && !isEpisodeInStrat) value.STRAT = value.IPP;
...
root.calculateCV = function(record, population, msrpopl, msrpoplex, observ, occurrenceId, value, stratification, variables, isEpisodeInStrat) {
  finalSpecifics = {};
