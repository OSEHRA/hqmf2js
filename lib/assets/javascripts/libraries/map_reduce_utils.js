...
root.isCriteriaInPopulation = function(measureId, population, criteria) {
  var measure = this.bonnie.measures.find(function(measure) {
    return measure.get('hqmf_id') == measureId
  });
  if(measure) {
    var populationCriteria = measure.get('population_criteria')[population]
    if (populationCriteria) {
      return criteria.some(function(childCriteria) {
        return isCriteriaInPrecondition(measure, populationCriteria, childCriteria);
      });
    }
  }
  return false;
}
root.isCriteriaInPrecondition = function(measure, precondition, criteria) {
  if(precondition.preconditions) {
    return precondition.preconditions.some(function(precondition) {
      return isCriteriaInPrecondition(measure, precondition, criteria);
    });
  } else if (precondition.reference) {
    return precondition.reference == criteria || isCriteriaInDataCriteria(measure, precondition.reference, criteria);
  }
}
root.isCriteriaInDataCriteria = function(measure, dataCriteriaId, criteria) {
  dataCriteria = measure.get('data_criteria')[dataCriteriaId];
  if(dataCriteria.children_criteria) {
    return dataCriteria.children_criteria.some(function(childCriteria) {
      return childCriteria == criteria || isCriteriaInDataCriteria(measure, childCriteria, criteria);
    });
  }
  return false;
}
