...
root.isCriteriaInPopulation = function(measureId, population, criteria) {
  var measure = null;
  for(var i = 0; i < this.bonnie.measures.length; i++) {
    if(this.bonnie.measures.models[i].get('hqmf_id') == measureId) {
      measure = this.bonnie.measures.models[i];
      break;
    }
  }
  var populationCriteria = measure.get('population_criteria')[population]
  if (populationCriteria) {
    for(var i = 0; i < criteria.length; i++) {
      return checkCriteriaInPrecondition(measure, populationCriteria, criteria[i]);
    }
  } else {
    return false;
  }
}
root.checkCriteriaInPrecondition = function(measure, precondition, criteria) {
  if(precondition.preconditions) {
    var result = false;
    for(var i = 0; i < precondition.preconditions.length; i++) {
      result = result || checkCriteriaInPrecondition(measure, precondition.preconditions[i], criteria);
    }
    return result;
  } else if (precondition.reference) {
    if(precondition.reference == criteria) {
      return true;
    } else {
      return checkCriteriaInDataCriteria(measure, precondition.reference, criteria)
    }
  }
}
root.checkCriteriaInDataCriteria = function(measure, dataCriteriaId, criteria) {
  dataCriteria = measure.get('data_criteria')[dataCriteriaId];
  if(dataCriteria.children_criteria) {
    var result = false;
    for(var i = 0; i < dataCriteria.children_criteria.length; i++) {
      if (dataCriteria.children_criteria[i] == criteria) {
        result = true;
        break;
      } else {
        result = result || checkCriteriaInDataCriteria(measure, dataCriteria.children_criteria[i], criteria);
      }
    }
    return result;
  } else {
...
root.calculate = function(record, population, denominator, numerator, exclusion, denexcep, occurrenceId, value, stratification, variables, numex) {
  ...
  if (ipp) {
    ...
    if (hqmf.SpecificsManager.validate(ipp)) {
      ...
      if (validStrat && value.STRAT == 0 && !isCriteriaInPopulation(value.measure_id, 'STRAT', occurrenceId)) value.STRAT = value.IPP;
